      ...
      <div class="section-title">
        ...
        <label class="filter">Show
          <select id="activity-filter">
            ...
            <option value="orders">Paper trades</option>
...
const paperDelta=(from,to,unit)=>{const value=integer(to)-integer(from);const amount=paperValue(value>=0n?value:-value,unit);return (value>=0n?'+':'-')+amount;};
...
function performanceChart(m){
  ...
  if(available.length<2)return '<div class="chart"><div class="chart-head"><span>Today</span><span>Worst fall '+paperValue(m.max_drawdown_micros,m.value_unit)+'</span></div><div class="chart-empty">Building today’s performance chart…</div></div>';
  ...
  const summary=m.name+' paper value moved from '+paperValue(first.equity_micros,m.value_unit)+' to '+paperValue(last.equity_micros,m.value_unit)+' ('+paperDelta(first.equity_micros,last.equity_micros,m.value_unit)+'). Holding moved from '+paperValue(first.hold_benchmark_micros,m.value_unit)+' to '+paperValue(last.hold_benchmark_micros,m.value_unit)+' ('+paperDelta(first.hold_benchmark_micros,last.hold_benchmark_micros,m.value_unit)+'). '+gaps+' unavailable interval'+(gaps===1?'':'s')+'.';
  return '<div class="chart"><div class="chart-head"><span>Today</span><span>Worst fall '+paperValue(m.max_drawdown_micros,m.value_unit)+'</span></div><p class="sr-only">'+safe(summary)+'</p><svg viewBox="0 0 100 56" preserveAspectRatio="none" aria-hidden="true" focusable="false"><line class="chart-grid" x1="0" y1="6" x2="100" y2="6"></line><line class="chart-grid" x1="0" y1="28" x2="100" y2="28"></line><line class="chart-grid" x1="0" y1="50" x2="100" y2="50"></line>'+hold+paper+'</svg><div class="chart-legend"><span>Paper</span><span>Holding</span></div></div>';
}
...
function metric(label,value,foot,klass=''){
  return '<article class="metric"><span class="metric-label">'+safe(label)+'</span><strong class="metric-value '+klass+'">'+safe(value)+'</strong><span class="metric-foot">'+safe(foot)+'</span></article>';
}
function renderMetrics(){
  if(!current.complete){$('metrics').innerHTML=metric('Paper value','—','Waiting for all markets')+metric("Today's paper P&L",'—','Waiting for current data')+metric('Compared with holding','—','Waiting for current data')+metric('Trades today','—','Waiting for current data');return;}
  ...
  $('metrics').innerHTML=metric('Paper value',paperValue(o.equity_micros,o.value_unit),'Across available markets')+
    metric("Today's paper P&L",paperDelta(o.opening_equity_micros,o.equity_micros,o.value_unit),'Current UTC paper period',tone(pnl))+
    metric('Compared with holding',paperDelta(o.hold_benchmark_micros,o.equity_micros,o.value_unit),'Same paper starting assets',tone(hold))+
    metric('Trades today',String(o.trades||0),String(o.signals||0)+' signals · '+(o.coverage_ready?'readable checks '+(Number(o.coverage_bps)/100).toFixed(2)+'%':'checks warming'));
}
function marketCard(m){
  ...
  const action=m.risk_halted?'No new orders':m.next_action?'Waiting to '+safe(m.next_action):state(m.state);
  const marketPrice=m.price_micros?(m.value_unit==='devUSDC'?'Reference '+price(m.price_micros):price(m.price_micros)):'Learning prices';
  return '<article class="market"><div class="market-head"><h3>'+safe(m.name)+'</h3>'+badge+'</div><p class="price">'+marketPrice+'</p><div class="market-state"><strong>'+safe(state(m.state))+'</strong><span>'+safe(action)+'</span></div><div class="market-metrics"><div><span>Paper value</span><strong>'+paperValue(m.equity_micros,m.value_unit)+'</strong></div><div><span>Today</span><strong class="'+tone(pnl)+'">'+paperDelta(m.opening_equity_micros,m.equity_micros,m.value_unit)+'</strong></div><div><span>Vs holding</span><strong class="'+tone(holding)+'">'+paperDelta(m.hold_benchmark_micros,m.equity_micros,m.value_unit)+'</strong></div><div><span>Activity</span><strong>'+safe(String(m.trades||0))+' trades · '+safe(String(m.signals||0))+' signals</strong></div></div>'+performanceChart(m)+'</article>';
}
function strategyCard(m){
  const unavailable=!m.available,label=unavailable?'Unavailable':m.ready?(m.strategy||'Saved plan'):'Updating';
  const next=unavailable?'Status source unavailable':m.ready?(m.next_action?'Next: '+m.next_action:'No next action yet'):'Waiting for status';
  const status=unavailable?'Unavailable':m.ready?state(m.state):'Status updating';
  const trades=unavailable||!m.ready?'—':(m.trades||0)+' trades · '+(m.signals||0)+' signals today';
  return '<article class="market"><div class="market-head"><h3>'+safe(m.name)+'</h3><span class="badge '+(unavailable?'red':'blue')+'">'+safe(label)+'</span></div><p class="price">'+safe(next)+'</p><div class="market-state"><strong>'+safe(status)+'</strong><span>'+safe(trades)+'</span></div></article>';
}
...
async function load(manual=false){
  ...
  try{const response=await fetch('/api/v1/status',{cache:'no-store'});if(!response.ok)throw new Error('status unavailable');current=await response.json();render();}
  catch(error){if(current){current.complete=false;current.markets.forEach(m=>m.fresh=false);render();}$('notice').textContent='Dashboard status is unavailable. It will retry automatically.';$('checked').textContent='Connection lost';$('connection-dot').className='dot bad';}
  finally{if(manual)$('refresh').disabled=false;}
}
load();setInterval(()=>{if(!document.hidden&&!document.activeElement.closest('.activity-list'))load();},10000);`
