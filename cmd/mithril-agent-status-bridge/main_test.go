...
func TestRunRequiresFixedCredentialAndActivatedListener(t *testing.T) {
	for _, args := range [][]string{
		nil,
		{"--credential", "other"},
		{"--credential", statusCredentialName, "extra"},
	...
	if err := run(t.Context(), []string{"--credential", statusCredentialName}, &bytes.Buffer{}); err == nil ||
...
func TestSystemdListenerRejectsMissingOrMismatchedActivation(t *testing.T) {
	t.Setenv("LISTEN_PID", "")
	t.Setenv("LISTEN_FDS", "")
	...
	t.Setenv("LISTEN_FDNAMES", "operator-status")
	if _, err := systemdUnixListener(); err == nil {
		t.Fatal("foreign activation PID accepted")
	}
	t.Setenv("LISTEN_PID", strconv.Itoa(os.Getpid()))
	t.Setenv("LISTEN_FDS", "2")
	if _, err := systemdUnixListener(); err == nil {
		t.Fatal("multiple activated sockets accepted")
	}
	t.Setenv("LISTEN_FDS", "1")
	t.Setenv("LISTEN_FDNAMES", "wrong-name")
	if _, err := systemdUnixListener(); err == nil {
		t.Fatal("mismatched activated socket name accepted")
	}
}
